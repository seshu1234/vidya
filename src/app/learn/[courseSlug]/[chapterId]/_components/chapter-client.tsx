...
export function ChapterClient({ course, chapterId, courseSlug }: ChapterClientProps) {
    ...
    course.syllabus?.forEach((week: { topics?: string[] }) => {
    ...
    if (currentIndex !== -1) {
        ...
        const week = course.syllabus?.find((w: { topics?: string[] }) => w.topics?.includes(activeTopic));
