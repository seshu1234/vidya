...
export async function getTools(search?: string) {
  ...
  let query = supabase.schema('vidya').from('tools').select('*').order('created_at', { ascending: false })
...
export async function updateTool(toolId: string, data: Record<string, unknown>) {
    const supabase = await createAdminClient()
    const { error } = await supabase
        .schema('vidya')
        .from('tools')
